...
import type { BeitraegeZahlungen, Veranstaltungen, Veranstaltungsteilnahmen, Mitglieder, CreateBeitraegeZahlungen, CreateVeranstaltungen, CreateVeranstaltungsteilnahmen, CreateMitglieder } from '@/types/app';
...
export class LivingAppsService {
  ...
  static async getBeitraegeZahlungen(): Promise<BeitraegeZahlungen[]> {
    ...
    return enrichLookupFields(records, 'beitraege_zahlungen');
  }
  static async getBeitraegeZahlungenEntry(id: string): Promise<BeitraegeZahlungen | undefined> {
    ...
    return enrichLookupFields([record], 'beitraege_zahlungen')[0];
  }
  static async createBeitraegeZahlungenEntry(fields: CreateBeitraegeZahlungen) {
    return callApi('POST', `/apps/${APP_IDS.BEITRAEGE_ZAHLUNGEN}/records`, { fields: cleanFieldsForApi(fields as any, 'beitraege_zahlungen') });
  }
  static async updateBeitraegeZahlungenEntry(id: string, fields: Partial<CreateBeitraegeZahlungen>) {
    return callApi('PATCH', `/apps/${APP_IDS.BEITRAEGE_ZAHLUNGEN}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'beitraege_zahlungen') });
  }
