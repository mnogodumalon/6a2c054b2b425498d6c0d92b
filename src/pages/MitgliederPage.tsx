...
export default function MitgliederPage() {
  ...
  const [records, setRecords] = useState<Mitglieder[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Mitglieder | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Mitglieder | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getMitglieder());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: Mitglieder['fields']) {
  ...
  async function handleUpdate(fields: Mitglieder['fields']) {
    ...
    await LivingAppsService.updateMitgliederEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteMitgliederEntry(deleteTarget.record_id);
  ...
  return (
    <PageShell
      title="Mitglieder"
      subtitle={`${records.length} Mitglieder im System`}
      action={
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('mitgliedsnummer')}>
                <span className="inline-flex items-center gap-1">
                  Mitgliedsnummer
                  {sortKey === 'mitgliedsnummer' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('mitgliedsstatus')}>
                <span className="inline-flex items-center gap-1">
                  Mitgliedsstatus
                  {sortKey === 'mitgliedsstatus' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('iban')}>
                <span className="inline-flex items-center gap-1">
                  IBAN (für Lastschrift)
                  {sortKey === 'iban' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; navigate(`/mitglieder/${record.record_id}`); }}>
                ...
                <TableCell>{record.fields.mitgliedsnummer ?? '—'}</TableCell>
                <TableCell className="text-muted-foreground">{formatDate(record.fields.eintrittsdatum)}</TableCell>
                <TableCell><span className="inline-flex items-center bg-secondary border border-[#bfdbfe] text-[#2563eb] rounded-[10px] px-2 py-1 text-sm font-medium">{record.fields.mitgliedsstatus?.label ?? '—'}</span></TableCell>
                <TableCell>{record.fields.iban ?? '—'}</TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={15} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Mitglieder. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
