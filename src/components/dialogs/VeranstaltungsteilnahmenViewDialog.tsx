...
interface VeranstaltungsteilnahmenViewDialogProps {
  open: boolean;
  onClose: () => void;
  record: Veranstaltungsteilnahmen | null;
  onEdit: (record: Veranstaltungsteilnahmen) => void;
  mitgliederList: Mitglieder[];
  veranstaltungenList: Veranstaltungen[];
}

export function VeranstaltungsteilnahmenViewDialog({ open, onClose, record, onEdit, mitgliederList, veranstaltungenList }: VeranstaltungsteilnahmenViewDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Veranstaltungsteilnahmen anzeigen</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-1">
            ...
            <p className="text-sm whitespace-pre-wrap">{record.fields.bemerkungen_teilnahme ?? '—'}</p>
          </div>
          <div className="pt-2 border-t border-border">
            <AttachmentsSection appId={APP_IDS.VERANSTALTUNGSTEILNAHMEN} recordId={record.record_id} readOnly />
          </div>
        </div>
      </DialogContent>
    </Dialog>
