...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/Veranstaltungsteilnahmen';
...
export function VeranstaltungsteilnahmenDialog({ open, onClose, onSubmit, defaultValues, recordId, mitgliederList, veranstaltungenList, enablePhotoScan = true, enablePhotoLocation = true }: VeranstaltungsteilnahmenDialogProps) {
  ...
  }), [mitgliederList, veranstaltungenList, ]);
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["mitglied", "veranstaltung"]);
        ...
        return merged as Partial<Veranstaltungsteilnahmen['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Veranstaltungsteilnahmen bearbeiten' : 'Veranstaltungsteilnahmen hinzufügen';
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'anmeldedatum': (
      <div key="anmeldedatum" className="space-y-1.5">
    ...
    'anwesenheit': (
      <div key="anwesenheit" className="space-y-1.5">
        <Label htmlFor="anwesenheit">Anwesenheit bestätigt</Label>
        <div className="flex items-center gap-2 pt-1">
          <Checkbox
            id="anwesenheit"
            checked={!!fields.anwesenheit}
            onCheckedChange={(v) => setFields(f => ({ ...f, anwesenheit: !!v }))}
          />
          <Label htmlFor="anwesenheit" className="font-normal">Anwesenheit bestätigt</Label>
        </div>
      </div>
    ),
    'bemerkungen_teilnahme': (
      <div key="bemerkungen_teilnahme" className="space-y-1.5">
  ...
  const FIELD_LABELS: Record<string, string> = {"mitglied": "Mitglied", "veranstaltung": "Veranstaltung", "anmeldedatum": "Anmeldedatum", "anwesenheit": "Anwesenheit bestätigt", "bemerkungen_teilnahme": "Bemerkungen"};
  const CURRENCY_KEYS = new Set<string>([]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"mitglied": {"geburtsdatum": "Geburtsdatum", "email": "E-Mail-Adresse", "telefon": "Telefonnummer", "strasse": "Straße", "hausnummer": "Hausnummer", "plz": "Postleitzahl", "ort": "Ort", "mitgliedsnummer": "Mitgliedsnummer", "eintrittsdatum": "Eintrittsdatum", "mitgliedsstatus": "Mitgliedsstatus", "iban": "IBAN (für Lastschrift)", "bemerkungen": "Bemerkungen", "vorname": "Vorname", "nachname": "Nachname"}, "veranstaltung": {"titel": "Titel der Veranstaltung", "beschreibung": "Beschreibung", "datum_uhrzeit": "Datum & Uhrzeit", "anmeldeschluss": "Anmeldeschluss", "veranstaltungsart": "Veranstaltungsart", "veranstaltungsort": "Veranstaltungsort", "max_teilnehmer": "Maximale Teilnehmerzahl", "verantwortlicher": "Verantwortliche Person", "bemerkungen_veranstaltung": "Bemerkungen"}};
