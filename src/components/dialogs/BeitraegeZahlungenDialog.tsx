...
import type { BeitraegeZahlungen, Mitglieder, LookupValue } from '@/types/app';
...
import { formEnhancements, computedDeps, computedApplookupRefs } from '@/config/form-enhancements/BeitraegeZahlungen';
...
interface BeitraegeZahlungenDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: BeitraegeZahlungen['fields']) => Promise<void>;
  /** SHAPE-TOLERANT: lookup fields accept the bare key (string) or the
   *  LookupValue object; applookup fields the bare record id or the full
   *  record URL — the dialog normalizes both. */
  defaultValues?: Omit<BeitraegeZahlungen['fields'], 'zahlungsart' | 'zahlungsstatus'> & {
    zahlungsart?: LookupValue | string;
...
const NORMALIZE_LOOKUPS: Record<string, readonly { key: string; label: string }[]> = {
  zahlungsart: LOOKUP_OPTIONS['beitraege_zahlungen']?.['zahlungsart'] ?? [],
  zahlungsstatus: LOOKUP_OPTIONS['beitraege_zahlungen']?.['zahlungsstatus'] ?? [],
};
...
export function BeitraegeZahlungenDialog({ open, onClose, onSubmit, defaultValues, recordId, mitgliederList, enablePhotoScan = true, enablePhotoLocation = true }: BeitraegeZahlungenDialogProps) {
  const [fields, setFields] = useState<Partial<BeitraegeZahlungen['fields']>>({});
  ...
  useEffect(() => {
    if (open) {
      setFields(applyDefaults(normalizedDefaults ?? {}, formEnhancements.defaults) as Partial<BeitraegeZahlungen['fields']>);
      setPreview(null);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const clean = cleanFieldsForApi(merged, 'beitraege_zahlungen');
      await onSubmit(clean as BeitraegeZahlungen['fields']);
      onClose();
    } catch (err) {
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "mitglied": string | null, // Display name from Mitglieder (see <available-records>)\n  "beitragsjahr": number | null, // Beitragsjahr\n  "beitragshoehe": number | null, // Beitragshöhe (€)\n  "zahlungsart": LookupValue | null, // Zahlungsart (select one key: "ueberweisung" | "lastschrift" | "bar") mapping: ueberweisung=Überweisung, lastschrift=Lastschrift, bar=Barzahlung\n  "zahlungsdatum": string | null, // YYYY-MM-DD\n  "zahlungsstatus": LookupValue | null, // Zahlungsstatus (select one key: "offen" | "bezahlt" | "gemahnt" | "storniert") mapping: offen=Offen, bezahlt=Bezahlt, gemahnt=Gemahnt, storniert=Storniert\n  "bemerkungen_zahlung": string | null, // Bemerkungen\n}`;
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["mitglied"]);
        ...
        return merged as Partial<BeitraegeZahlungen['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Beiträge & Zahlungen bearbeiten' : 'Beiträge & Zahlungen hinzufügen';
  ...
  const FIELD_LABELS: Record<string, string> = {"mitglied": "Mitglied", "beitragsjahr": "Beitragsjahr", "beitragshoehe": "Beitragshöhe (€)", "zahlungsart": "Zahlungsart", "zahlungsdatum": "Zahlungsdatum", "zahlungsstatus": "Zahlungsstatus", "bemerkungen_zahlung": "Bemerkungen"};
  const CURRENCY_KEYS = new Set<string>(["beitragshoehe"]);
  // Applookup-Referenz-Labels: pro applookup-Feld in dieser Form (ownKey)
  // eine Map { lookupKey: label } für ALLE Felder des Target-Schemas. Wird
  // beim Render-Walk gefiltert auf die in der computed-Formel tatsächlich
  // referenzierten lookupKeys (siehe applookupRefs unten).
  const APPLOOKUP_LABELS: Record<string, Record<string, string>> = {"mitglied": {"geburtsdatum": "Geburtsdatum", "email": "E-Mail-Adresse", "telefon": "Telefonnummer", "strasse": "Straße", "hausnummer": "Hausnummer", "plz": "Postleitzahl", "ort": "Ort", "mitgliedsnummer": "Mitgliedsnummer", "eintrittsdatum": "Eintrittsdatum", "mitgliedsstatus": "Mitgliedsstatus", "iban": "IBAN (für Lastschrift)", "bemerkungen": "Bemerkungen", "vorname": "Vorname", "nachname": "Nachname"}};
