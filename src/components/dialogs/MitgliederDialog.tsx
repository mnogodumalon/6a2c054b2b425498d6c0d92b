...
export function MitgliederDialog({ open, onClose, onSubmit, defaultValues, recordId, enablePhotoScan = true, enablePhotoLocation = true }: MitgliederDialogProps) {
  ...
  const REQUIRED_FIELDS = ['email', 'eintrittsdatum', 'mitgliedsstatus', 'vorname', 'nachname'] as const;
  ...
  const DIALOG_INTENT = defaultValues ? 'Mitglieder bearbeiten' : 'Mitglieder hinzufügen';
  ...
  const fieldBlocks: Record<string, React.ReactNode> = {
    ...
    'eintrittsdatum': (
      <div key="eintrittsdatum" className="space-y-1.5">
        <Label htmlFor="eintrittsdatum">Eintrittsdatum <span className="text-destructive" aria-hidden="true">*</span></Label>
        <DatePicker
          id="eintrittsdatum"
          placeholder=""
          mode="date"
    ...
    'mitgliedsstatus': (
      <div key="mitgliedsstatus" className="space-y-1.5">
        <Label htmlFor="mitgliedsstatus">Mitgliedsstatus <span className="text-destructive" aria-hidden="true">*</span></Label>
        <div role="radiogroup" className="flex flex-wrap gap-1.5">
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.mitgliedsstatus) === 'passiv'}
            onClick={() => setFields(f => ({ ...f, mitgliedsstatus: (lookupKey(f.mitgliedsstatus) === 'passiv' ? undefined : 'passiv') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.mitgliedsstatus) === 'passiv'
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.mitgliedsstatus) === 'ehrenmitglied'}
            onClick={() => setFields(f => ({ ...f, mitgliedsstatus: (lookupKey(f.mitgliedsstatus) === 'ehrenmitglied' ? undefined : 'ehrenmitglied') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.mitgliedsstatus) === 'ehrenmitglied'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.mitgliedsstatus) === 'ausgetreten'}
            onClick={() => setFields(f => ({ ...f, mitgliedsstatus: (lookupKey(f.mitgliedsstatus) === 'ausgetreten' ? undefined : 'ausgetreten') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.mitgliedsstatus) === 'ausgetreten'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
          ...
          <button
            type="button"
            role="radio"
            aria-checked={lookupKey(fields.mitgliedsstatus) === 'aktiv'}
            onClick={() => setFields(f => ({ ...f, mitgliedsstatus: (lookupKey(f.mitgliedsstatus) === 'aktiv' ? undefined : 'aktiv') as any }))}
            className={`inline-flex items-center justify-center min-h-9 max-sm:min-h-11 max-sm:px-4 rounded-full border px-3 py-1.5 text-sm font-medium transition-colors ${
              lookupKey(fields.mitgliedsstatus) === 'aktiv'
                ? 'bg-foreground text-background border-foreground'
                : 'bg-background text-foreground border-input hover:bg-accent'
            }`}
          >
        ...
        {showErrors && !fields.mitgliedsstatus && (
          <p className="text-xs text-destructive mt-1">Pflichtfeld</p>
        )}
      </div>
    ),
